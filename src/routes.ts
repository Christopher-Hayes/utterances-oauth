...
async function authorizedRequestHandler(search: URLSearchParams) {
  ...
  try {
    const response = (await fetch(accessTokenUrl, init)) as {
      ok: boolean;
...
async function postIssueRequestHandler(
  ...
  try {
    const response = (await fetch("https://api.github.com/user", authInit)) as {
      ok: boolean;
    };
  ...
  try {
    const response = (await fetch(`https://api.github.com${path}`, init)) as {
      status: number;
      statusText: string;
      json: () => Promise<{ number: number }>;
    };
